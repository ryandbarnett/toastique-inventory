// tests/api/v1/juices.sort.spec.mjs
...
describe('GET /api/v1/juices sorting', () => {
  ...
  it('sort=name&dir=asc returns names ascending', async () => {
    const res = await agent.get(api.juices.list('?sort=name&dir=asc')).expect(200)
  ...
  it('sort=name&dir=desc returns names descending', async () => {
    const res = await agent.get(api.juices.list('?sort=name&dir=desc')).expect(200)
    const names = res.body.map(j => j.name)
  ...
  it('sort=status&dir=asc ranks OUT < BELOW PAR < OK', async () => {
    const res = await agent.get(api.juices.list('?sort=status&dir=asc')).expect(200)
  ...
  it('sort=status&dir=desc ranks OK < BELOW PAR < OUT', async () => {
    const res = await agent.get(api.juices.list('?sort=status&dir=desc')).expect(200)
    const ranks = res.body.map(j => rank[j.status])
