...
describe('PUT /api/v1/juices/:id/liters', () => {
  ...
  it('updates currentLiters, returns updated record, and bumps lastUpdated (list-only flow)', async () => {
    ...
    const putRes = await api.put(`/api/v1/juices/${j0.id}/liters`).send({ liters: newLiters }).expect(200)
  ...
  it('always bumps lastUpdated even if liters did not change', async () => {
    const list = await api.get('/api/v1/juices').expect(200)
    const j0 = list.body[0]
    const sameLiters = j0.currentLiters

    const res1 = await api.put(`/api/v1/juices/${j0.id}/liters`).send({ liters: sameLiters }).expect(200)
    ...
    const res2 = await api.put(`/api/v1/juices/${j0.id}/liters`).send({ liters: sameLiters }).expect(200)
  ...
  it('rejects invalid liters (non-number, negative, missing, null, boolean, object, >30 cap)', async () => {
    ...
    await api.put(`/api/v1/juices/${id}/liters`).send({ liters: '5' }).expect(400)   // string
    await api.put(`/api/v1/juices/${id}/liters`).send({ liters: -1 }).expect(400)    // negative
    await api.put(`/api/v1/juices/${id}/liters`).send({}).expect(400)                // missing
    await api.put(`/api/v1/juices/${id}/liters`).send({ liters: null }).expect(400)  // null
    await api.put(`/api/v1/juices/${id}/liters`).send({ liters: true }).expect(400)  // boolean
    await api.put(`/api/v1/juices/${id}/liters`).send({ liters: { n: 5 } }).expect(400) // object
    await api.put(`/api/v1/juices/${id}/liters`).send({ liters: 31 }).expect(400)    // >30 cap
    // Note: JSON.stringify(Infinity/NaN) => null, so covered by the 'null' case.
  })

  it('404s on unknown juice id', async () => {
    await api.put('/api/v1/juices/999999/liters').send({ liters: 1 }).expect(404)
  })
