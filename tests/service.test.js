...
describe('service.createBatch', () => {
  ...
  it('rejects non-numeric sizes', async () => {
    await expect(service.createBatch(1, { size: '1.5x' }))
      .rejects.toThrow(/size must be 1 or 1\.5/i)
  })

  it('rejects numeric sizes other than 1 or 1.5', async () => {
    await expect(service.createBatch(1, { size: 2 }))
      .rejects.toThrow(/size must be 1 or 1\.5/i)
    await expect(service.createBatch(1, { size: 1.2 }))
      .rejects.toThrow(/size must be 1 or 1\.5/i)
  })
})
